<div class="row mt-3">
    <div class="col-sm-12 bg-secondary">
        <div class="connect-sorting">
            <h5 class="text-center mb-2">DENOMINACIONES</h5>
            <div class="container">
                <div class="row">
                    @foreach ($denominations as $d)
                    <div class="col-sm mt-2">
                        <button wire:click.prevent="ACash({{$d->value}})" class="btn btn-dark btn-block den">
                            {{$d->value>0 ? '$' . number_format($d->value,2, '.', '') : 'Exacto' }}
                        </button>
                    </div>
                    @endforeach
                </div>
            </div>
            <div class="connect-sorting-content mt-4">
                <div class="card simple-title-task ui-sortable-handle">
                    <div class="card-body">
                        <div class="input-group input-group-md mb-3">
                            <div class="input-group-prepend">
                                <span class="input-group-text input-gp hideonsm" style="background: #3B3F5C;
                                color:white">EFECTIVO F8
                                </span>
                            </div>
                            <input type="number" id="cash"
                            wire:model="efectivo"
                            wire:keydown.enter="saveSale"
                            class="form-control text-center" value="{{$efectivo}}"
                            >
                            <div class="input-group-append">
                                <span wire:click="$set('efectivo', 0)" class="input-text" style="
                                background: #3B3F5C; color:white">
                                        <i class="fas fa-backspace fa-2x"></i>
                                </span>
                            </div>
                        </div>
                        <h4 class="text-muted">Cambio: ${{number_format($change,2)}}</h4>
                        <div class="row justify-content-between mt-5">
                            <div class="col-sm-12 col-md-12 col-lg-6">
                                @if ($total>0)
                                <button onclick="Confirm('', 'clearCart', 'Segur@ de eliminar el carrito?')" class="btn btn-dark mtmobile">
                                    CANCELAR F4
                                </button>
                                @endif

                            </div>
                            <div class="col-sm-12 col-md-12 col-lg-6">
                                @if ($total>0 && $efectivo>=$total)
                                <button wire:click.prevent="saveSale('', 'clearCart', 'Seguro eliminar el carrito?')" class="btn btn-dark btn-md btn-block">
                                    GUARDAR F9
                                </button>
                                @endif
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>
{{-- <div class="card card-secondary">
    <div class="card-header">
        {{-- <h5 class="card-title">RESUMEN DE VENTA</h5> --}}
        {{-- <h6 class="text-center">DENOMINACIONES</h6>
    </div> --}}
    <!-- /.card-header -->
    {{-- <div class="card-body">
        <div>
            <h2>TOTAL: ${{number_format($total,2)}}</h2>
            <input type="hidden" id="total" name="hiddenTotal" value="{{$total}}">
        </div>
        <div>
            <h4 class="mt-3">Articulos: {{$itemsQuantity}}</h4>
        </div>
    </div> --}}
    <!-- /.card-body -->
{{-- </div>  --}}
{{-- <div class="card card-row card-default bg-secondary">
    <div class="card-header bg-info">
      <h3 class="card-title" _msthash="1837563" _msttexthash="97513"> En curso </h3>
    </div>
    <div class="card-body">
              <div class="card-body bg-primary">
          <p _msthash="2624700" _msttexthash="19610864"> Lorem ipsum dolor sit amet, consectetuer adipiscing elit. Eneano commodo ligula eget dolor. Massa enea. Cum sociis natoque penatibus et magnis dis parturient montes, nascetur ridiculus mus. </p>
        </div>
      </div>
    </div>
  </div> --}}
